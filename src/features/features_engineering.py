...
def add_technical_features(df):
    ...
    Includes: Technical Indicators + Time Features.
    NO Raw News Data (as requested).
    """
    df = df.copy()
    
    # 1. Standardize Input Columns
    ...
    # --- 2. Technical Features (Relative Indicators) ---
    ...
    # 🔥 [NEW ADDITION] Time Features (时间特征) 🔥
    # 捕捉市场的时间规律 (例如: 21:00-23:00 波动大)
    if "Date" in df.columns:
        df["Hour"] = df["Date"].dt.hour
        df["DayOfWeek"] = df["Date"].dt.dayofweek
        # 循环编码 (让 AI 知道 23点 和 0点 很近)
